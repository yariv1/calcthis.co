...
- Cross-links: `.pill` — never custom classes. REQUIRED: a `<div class="related-calcs">` block
  (heading `.related-calcs-h` + `<div class="pills">` of 2–4 related calcs) right after
  `</div><!-- grid -->`, before `<section class="content">`. Plus a contextual inline link in a
  results/advanced note where it earns its place. See `calcthis-design-system.md` → "Cross-links".
