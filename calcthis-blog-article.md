...
## Images — permanent location (never ask, never change)

All article images are always at:
`C:\Users\yariv\Utility Website\calcthis\assets\images`

Hero image naming: `blog-{topic}-article-header.webp`
Card image naming: `blog-{topic}-article-card.webp`

Images are already in the repo at the correct path. No copy step in CC prompt. Ever.
...
- [ ] Images already in repo at `assets/images/` — no action needed
- [ ] Run `node build.js` — all pages ✓, no warnings
- [ ] Zip and deploy

---

## CC deploy prompt (exact, every time)

```
Unzip ~/Downloads/calcthis-vN.zip into the repo root (overwrite existing files), run node build.js, then commit all changes with the message "COMMIT MESSAGE" and push to main.
```
